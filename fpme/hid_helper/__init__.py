...
abs_path = os.path.join(os.path.dirname(__file__), 'hidapi-win', 'x64', 'hidapi.dll')
hidapi = ctypes.cdll.LoadLibrary(abs_path)
...
if __name__ == '__main__':
    for dev_info in enumerate():
        vid = dev_info['vendor_id']
        pid = dev_info['product_id']
        name = dev_info['path']
        device = Device(path=name)
        # device = hid.Device(vid, pid)
        print(device.product, device.manufacturer, vid, pid, name)
        try:
            device.read(1, timeout=100)
            print(" +")
        except HIDException as exc:
            print("-", exc)
