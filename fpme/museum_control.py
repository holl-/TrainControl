...
def both_outside(pause: float, pause_random: float, rounds=1):
    ...
    for i in range(rounds):
        ...
        if i > 0:  # if i==0, GTO is already in Munich
            GTO.drive(O_MUNICH, pause=pause + random() * pause_random, trip=[(OUTER_CONTACT, O_CONTACT_NORTH - TRAIN_CONTACT)])
        IGBT.drive(O_AIRPORT + 80, pause=pause + random() * pause_random, wait_for='brake')
        ...
        GTO.drive(O_AIRPORT + 80, pause=pause + random() * pause_random, wait_for='brake')
        IGBT.drive(OUTER + O_ERDING - 80, pause=pause + random() * pause_random, wait_for='brake')
